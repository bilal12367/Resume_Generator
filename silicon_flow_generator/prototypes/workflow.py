...
class MyWorkflow(Workflow):
    ...
    # ------------------------------------------------------------------
    @step
    async def collector(self, ctx: Context, ev: ColorDoneEvent | LayoutDoneEvent) -> GenerateHeaderEvent | GenerateExperienceEvent | GenerateProjectsEvent:
        ...
        ctx.send_event(GenerateHeaderEvent(colors=colors, layout=layout))
    ...
    # ------------------------------------------------------------------
    @step
    async def header_generator(self, ctx: Context, ev: GenerateHeaderEvent) -> HeaderDoneEvent:
        try:
            prompts: dict   = await ctx.store.get("prompts")
            user_data: dict = await ctx.store.get("user_data")

            header_prompt: str = prompts.get("header_generation")
            if not header_prompt:
                raise ValueError("prompts dict is missing 'header_generation' key.")
            ...
            llm    = LLM()
            result = llm.call(user_prompt=filled_prompt, max_tokens=4096)

            ctx.write_event_to_stream(LogEvent(
                message="[header_generator] Done.",
                object={"header_html": result[:120]}
            ))

            return HeaderDoneEvent(html=result)

        except Exception as e:
            ctx.write_event_to_stream(LogEvent(message=f"[header_generator] Error: {e}"))
            raise

    # ------------------------------------------------------------------
    # Node 4b — experience generator (parallel)
    # ------------------------------------------------------------------
    @step
    async def experience_generator(self, ctx: Context, ev: GenerateExperienceEvent) -> ExperienceDoneEvent:
        try:
            prompts: dict   = await ctx.store.get("prompts")
            user_data: dict = await ctx.store.get("user_data")

            experience_prompt: str = prompts.get("experience_generation")
            if not experience_prompt:
                raise ValueError("prompts dict is missing 'experience_generation' key.")
            ...
            ctx.write_event_to_stream(LogEvent(
                message="[experience_generator] Done.",
                object={"experience_html": result}
            ))

            return ExperienceDoneEvent(html=result)

        except Exception as e:
            ctx.write_event_to_stream(LogEvent(message=f"[experience_generator] Error: {e}"))
            raise
    ...
    # ------------------------------------------------------------------
    @step
    async def projects_generator(self, ctx: Context, ev: GenerateProjectsEvent) -> ProjectsDoneEvent:
        try:
            prompts: dict   = await ctx.store.get("prompts")
            user_data: dict = await ctx.store.get("user_data")

            project_detail_prompt: str = prompts.get("project_details")
            if not project_detail_prompt:
                raise ValueError("prompts dict is missing 'project_details' key.")

            filled_prompt = (
                project_detail_prompt
                .replace("{{colors}}",    ev.colors)
                .replace("{{layout}}",    ev.layout)
                .replace("{{user_data}}", str(user_data))
            )
            ...
            llm    = LLM()
            result = llm.call(user_prompt=filled_prompt, max_tokens=8196)

            ctx.write_event_to_stream(LogEvent(
                message="[project_detail_generator] Done.",
                object={"project_detail_generator": result}
            ))

            return ProjectsDoneEvent(html=result)

        except Exception as e:
            ctx.write_event_to_stream(LogEvent(message=f"[project_detail_generator] Error: {e}"))
            raise
    # ------------------------------------------------------------------
    # Node 5 — final collector: waits for header + experience, returns both
    # ------------------------------------------------------------------
    @step
    async def final_collector(self, ctx: Context, ev: HeaderDoneEvent | ExperienceDoneEvent | ProjectsDoneEvent) -> StopEvent:
        collected = ctx.collect_events(ev, [HeaderDoneEvent, ExperienceDoneEvent, ProjectsDoneEvent])
        if collected is None:
            return None

        header_ev, experience_ev, project_ev = collected[0], collected[1], collected[2]
        ...
        return StopEvent(result={
            "header":     header_ev.html,
            "experience": experience_ev.html,
            "project": project_ev.html
        })
import json
...
async def main():
    ...
    header_html = str(final_result.get('header', '')).replace('\n', '')
    experience_html = str(final_result.get('experience', '')).replace('\n', '')
