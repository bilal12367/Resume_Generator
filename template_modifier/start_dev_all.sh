...
if command -v gnome-terminal &> /dev/null && [ -n "$DISPLAY" -o -n "$WAYLAND_DISPLAY" ]; then
    ...
    gnome-terminal --title="Centrifugo WS (8002)" -- bash -c "echo 'Starting Centrifugo...' && docker compose -f dev_containers/centrifugo-compose.yaml up; exec bash"
    gnome-terminal --title="LinkedIn MCP (8000)" -- bash -c "echo 'Starting LinkedIn MCP Server...' && uv run mcps/linkedin_platform.py; exec bash"
    gnome-terminal --title="Job Scraper API (8080)" -- bash -c "echo 'Starting Job Scraper API...' && uv run -m module_testing.job_scraper_2; exec bash"
    gnome-terminal --title="React UI (5173)" -- bash -c "cd job_scraper_ui && echo 'Starting Vite UI...' && npm run dev; exec bash"
